...
class VTGrepStrings(idaapi.action_handler_t):
    ...
    def activate(cls, ctx):
        for idx in ctx.chooser_selection:
            _, _, _, selected_string = ida_kernwin.get_chooser_data(ctx.widget_title, idx)
    ...
    def register(cls, plugin, label):
        ...
        return idaapi.register_action(idaapi.action_desc_t(cls.get_name(), instance.get_label(), instance))
...
class Popups(idaapi.UI_Hooks):
    ...
    def finish_populating_widget_popup(form, popup):
        if idaapi.get_widget_type(form) == idaapi.BWN_DISASM:
            idaapi.attach_action_to_popup(form, popup, VTGrepBytes.get_name(), "VirusTotal/")
            ...
            idaapi.attach_action_to_popup(form, popup, VTGrepStrings.get_name(), "VirusTotal/")
...
class CheckSample(threading.Thread):
    ...
    def check_file_missing_in_VT(self):
        ...
        if config.API_KEY:
            ...
            headers = {"User-Agent": user_agent, "Accept": "application/json", "x-apikey": config.API_KEY}
...
class VTpluginSetup(object):
    ...
    def write_config(self):
        ...
        try:
            ...
            logging.exception("[VT Plugin] Error while creating the user config file.")
            return False
        return True
    ...
    def __compare_versions(self, current, new):
        ...
        if new_ver[0] > current_ver[0] or (new_ver[0] == current_ver[0] and new_ver[1] > current_ver[1]):
            return True
        return False
    ...
    def __init__(self, cfgfile):
        ...
        logging.info("\n** VT Plugin for IDA Pro v%s (c) Google, 2023", VT_IDA_PLUGIN_VERSION)
...
class VTplugin(idaapi.plugin_t):
    ...
    def init(self):
        ...
        if valid_config:
            ...
            try:
                if procname in self.SEARCH_STRICT_SUPPORTED:
                ...
                elif procname in self.SEARCH_CODE_SUPPORTED:
                    VTGrepWildcards.register(self, "Search for similar code")
                    VTGrepWildCardsFunction.register(self, "Search for similar functions")
                else:
                    logging.info("\n - Processor detected: %s", procname)
                ...
                logging.exception("[VT Plugin] Unable to register popups actions.")
        else:
    ...
    def search_with_wildcards(strict):
        search_vt = vtgrep.VTGrepSearch(addr_start=idc.read_selection_start(), addr_end=idc.read_selection_end())
    ...
    def search_function_with_wildcards():
        ...
        if not addr_func:
            ...
            search_vt = vtgrep.VTGrepSearch(addr_start=addr_func.start_ea, addr_end=addr_func.end_ea)
